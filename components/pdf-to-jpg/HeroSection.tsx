...
import { ArrowRight, FileText, Upload, Loader2, X, Download, Image as ImageIcon, ChevronLeft, ChevronRight } from "lucide-react";
...
export default function HeroSection() {
  ...
  // 将 PDF 页面转换为 JPG
  // 完全按照 bentopdf-main/src/js/logic/pdf-to-jpg.ts 的实现
  const convertPdfToJpg = useCallback(async (file: File) => {
    if (!validateFile(file)) return;

    setIsProcessing(true);
    try {
      // 按照 bentopdf-main 的方式：使用 getPDFDocument 辅助函数
      ...
      // 按照 bentopdf-main 的方式：传递 canvas 参数
      await page.render({
        ...
        const imageDataUrl = URL.createObjectURL(jpegBlob);
        pages.push({
          pageNumber: pageNum,
          imageDataUrl,
          blob: jpegBlob,
  ...
  // 下载单个 JPG
  const downloadSingleJpg = useCallback((page: ConvertedPage) => {
  ...
  const downloadAllAsZip = useCallback(async () => {
    ...
    try {
      ...
      convertedPages.forEach((page) => {
        zip.file(`page_${page.pageNumber}.jpg`, page.blob);
      });
