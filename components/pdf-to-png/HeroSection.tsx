...
export default function HeroSection() {
  const t = useTranslations("PdfToPng");
  ...
  // 将 PDF 页面转换为 PNG
  const convertPdfToPng = useCallback(async (file: File) => {
    ...
    try {
      ...
      // 根据质量设置确定 scale（PNG 是无损的，只调整分辨率）
      let scale = 2.0;
      if (imageQuality === "low") {
        scale = 1.0;
  ...
  // 下载单个 PNG
  const downloadSinglePng = useCallback((page: ConvertedPage) => {
    const url = URL.createObjectURL(page.blob);
    const a = document.createElement("a");
    a.href = url;
    a.download = `page_${page.pageNumber}.png`;
  ...
  }, [imageQuality, pdfFile, convertPdfToPng, convertedPages.length]);
                  ...
                  {convertedPages.map((page, index) => (
                    ...
                    >
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <Button
                          onClick={() => downloadSinglePng(page)}
                          size="sm"
                          variant="outline"
                          className="h-8"
                        >
