...
export default function CircleCrop() {
  ...
  const constrainOffsets = (img: HTMLImageElement, z: number, ox: number, oy: number) => {
    ...
    // 对于较大的图片（大于圆形裁剪区域），允许在 VIEWPORT_SIZE 范围内移动
    // 对于较小的图片（小于圆形裁剪区域），允许在 BOUNDARY_SIZE 范围内移动
    if (scaledW > VIEWPORT_SIZE) {
      // 图片大于圆形裁剪区域，限制在 VIEWPORT_SIZE 范围内移动
      const maxOff = (scaledW - VIEWPORT_SIZE) / 2;
      newOx = Math.max(-maxOff, Math.min(maxOff, ox));
    } else {
      // 图片小于圆形裁剪区域，允许在 BOUNDARY_SIZE 范围内移动
      // 限制：图片边缘不能超出边界
      const maxOffX = (BOUNDARY_SIZE - scaledW) / 2;
      newOx = Math.max(-maxOffX, Math.min(maxOffX, ox));
    }
    ...
    if (scaledH > VIEWPORT_SIZE) {
      ...
      // 图片小于圆形裁剪区域，允许在 BOUNDARY_SIZE 范围内移动
      // 限制：图片边缘不能超出边界
      const maxOffY = (BOUNDARY_SIZE - scaledH) / 2;
      newOy = Math.max(-maxOffY, Math.min(maxOffY, oy));
    }
  ...
  const renderCrop = () => {
    ...
    // 绘制圆形裁剪区域的遮罩
    const centerX = BOUNDARY_SIZE / 2;
    const centerY = BOUNDARY_SIZE / 2;
    const radius = VIEWPORT_SIZE / 2;

    // 绘制半透明遮罩（圆形外部）
    ctx.fillStyle = "rgba(0, 0, 0, 0.5)";
    ctx.beginPath();
    ctx.rect(0, 0, BOUNDARY_SIZE, BOUNDARY_SIZE);
    ...
    try {
      ...
      finalCanvas.toBlob(
        (blob) => {
          ...
          a.download = `circle-crop.${extension}`;
          a.href = url;
          a.click();
  ...
  // 暴露文件输入给外部（HeroSection 按钮可以触发）
  useEffect(() => {
    if (typeof window !== "undefined") {
      (window as any).triggerCircleCropUpload = () => {
        fileInputRef.current?.click();
      };
    }
    return () => {
      if (typeof window !== "undefined") {
        delete (window as any).triggerCircleCropUpload;
      }
    };
  }, []);
