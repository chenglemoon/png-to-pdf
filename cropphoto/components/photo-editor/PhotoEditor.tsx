...
const FilerobotImageEditorWrapperDynamic = dynamic(
    () => import('./FilerobotImageEditorWrapper'),
    { ssr: false, loading: () => <div className="flex items-center justify-center h-[700px]">Loading editor...</div> }
);
...
export default function PhotoEditor() {
    ...
    // 使用 public 目录下的 circleimage-4.jpg 作为默认图片
    const [photoUrl, setPhotoUrl] = useState<string | null>('/circleimage-4.jpg');
    const [photoName, setPhotoName] = useState('circleimage-4.jpg');
        ...
        <div className="w-full max-w-7xl mx-auto">
            <div className="flex justify-center items-center gap-2 mb-6">
                <div className="relative">
                    <button 
                        className="py-1 flex gap-1 items-center px-4 rounded-full text-sm border-0 bg-[#6879eb] text-white hover:bg-[#5a6bd8] transition-colors" 
                    ...
                    />
                    <span className="absolute text-xs opacity-60 top-8 left-[50%] translate-x-[-50%] whitespace-nowrap">
                        {t("orPaste")}
                    </span>
            ...
            <div className="h-[calc(100vh-300px)] min-h-[700px] max-h-[900px] w-full rounded-md shadow-lg">
                {isReader && photoUrl && (
                    <FilerobotImageEditorWrapperDynamic
                        source={photoUrl}
