...
export default function FAQ() {
  const t = useTranslations("ImageCompressor.faq");
                ...
                {faqs.map((faq, index) => {
                  ...
                  return (
                    <div key={faq.id} className="relative">
                      ...
                      >
                        <span className="text-base md:text-lg font-semibold text-gray-900 dark:text-white pr-6 flex-1 leading-snug group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors">
                          {t(faq.questionKey)}
                        </span>
                        <ChevronUp 
                          className={`w-5 h-5 text-gray-500 dark:text-gray-400 flex-shrink-0 mt-0.5 transition-all duration-200 ${isExpanded ? 'text-blue-600 dark:text-blue-400' : 'rotate-180 group-hover:text-gray-700 dark:group-hover:text-gray-300'}`}
                        />
                      </button>
                      {isExpanded && (
                        <motion.div
