...
import { ShapeLine } from './beautifier/Shape';
import { Icon } from './beautifier/Icons';
import { Button, Slider, Radio, Select, Switch, Upload, Watermark, Input, ColorPicker, message } from 'antd';
import { Icons } from './beautifier/Icons';
import { cn, supportImg, toDownloadFile, copyAsBlob, getBackground, captureScreen } from '@/lib/beautifier-utils';
import backgroundConfig from '@/lib/beautifier-backgroundConfig';
import { MacbookPro } from './beautifier/MacbookPro';
import { IphonePro } from './beautifier/IphonePro';
import { SelectDropdown } from './beautifier/SelectDropdown';
import { Toolbar } from './beautifier/Toolbar';
import useKeyboardShortcuts from '@/lib/beautifier-useKeyboardShortcuts';
import usePaste from '@/lib/beautifier-usePaste';
import useImageColor, { type ImageSize } from '@/lib/beautifier-useImageColor';
import useAnnotate, { type Shape } from '@/lib/beautifier-useAnnotate';
import { DrawerSide } from './beautifier/DrawerSide';
import { DownBtn } from './beautifier/DownBtn';
...
export default function Beautifier() {
    ...
    const [waterCont, setWaterCont] = useState('CircleImage');
    ...
    const annotateResult = useAnnotate();
    const canvasRef = annotateResult.canvasRef as React.RefObject<any>;
    const {
        shapes,
        setShapes,
        annotateWidth,
    ...
    const boxStyle = useMemo(() => {
        let style = {};
        if (bgValue in backgroundConfig) return style;
        try {
    ...
    useEffect(() => {
        if (ratioSize.key === 'auto' && imgSize.width && imgSize.height) setRatio(imgSize.width / imgSize.height);
    }, [imgSize, ratioSize]);
    ...
    usePaste((file: File) => {
        toRest();
        toClearAll();
        if (!(bgValue in backgroundConfig)) setBgValue('default_1');
    ...
    const handleColorChange = (color: string | { toRgbString: () => string }) => {
        setWaterColor(typeof color === 'string' ? color : color.toRgbString());
    }

    const mergeCanvas = (): Promise<HTMLCanvasElement> => new Promise((resolve, reject) => {
        if (!boxRef.current) {
            reject(new Error('Box ref is not available'));
            return;
        }
        toRest();
        toCanvas(boxRef.current, {
            cacheBust: false,
            canvasWidth: ratioSize.key === 'auto' ? (imgSize.width ?? ratioSize.width) : ratioSize.width,
            canvasHeight: ratioSize.key === 'auto' ? (imgSize.height ?? ratioSize.height) : ratioSize.height,
            ...
            const annotate = canvasRef.current.toCanvas({
                pixelRatio: (ratioSize.key === 'auto' ? (imgSize.width ?? ratioSize.width) : ratioSize.width) / boxWidth
            });
    ...
    const toDownload = () => {
        ...
        mergeCanvas().then(canvas => {
            ...
            toDownloadFile(dataUrl, 'circleImage.png');
    ...
    const onSelectDropdownClick = (item: {
        ...
        if (!item) return;
        const { label, key, width, height, icon } = item;
        setRatioSize({ 
            key, 
        ...
        if (key === 'auto') {
            if (imgSize.width && imgSize.height) setRatio(imgSize.width / imgSize.height);
        } else {
            if (width && height) {
                    ...
                    <div className="md:w-0 md:flex-1 overflow-auto select-none p-5">
                        <div className="min-w-full md:min-w-[600px] min-h-full flex items-center justify-center">
                            <div ref={boxRef} className={cn(
                                ...
                                {bgValue.includes('_img_') && bgValue in backgroundConfig && <img src={`${(backgroundConfig as Record<string, string>)[bgValue]}&w=1200`} className="w-full h-full absolute z-0 object-cover object-center" />}
                                <Watermark
                    ...
                    <div className="bg-white flex flex-col gap-2 p-4 md:w-[340px] border-l border-l-gray-50 shadow-lg relative z-10">
                            ...
                            <div className="py-3">
                                <Radio.Group onChange={(e) => setBgValue(e.target.value)} value={bgValue} rootClassName="grid grid-cols-7 [&_span]:ps-0">
                                    <Radio className="[&_.ant-radio]:hidden [&_span]:p-0 mr-0" value='default_1'>
                                        <div className={cn("w-8 h-8 rounded-full overflow-hidden", backgroundConfig.default_1)}></div>
                                    </Radio>
                                    <Radio className="[&_.ant-radio]:hidden [&_span]:p-0 mr-0" value='default_2'>
                                        <div className={cn("w-8 h-8 rounded-full overflow-hidden", backgroundConfig.default_2)}></div>
                                    </Radio>
                                    <Radio className="[&_.ant-radio]:hidden [&_span]:p-0 mr-0" value='default_3'>
                                        <div className={cn("w-8 h-8 rounded-full overflow-hidden", backgroundConfig.default_3)}></div>
                                    </Radio>
                                    <Radio className="[&_.ant-radio]:hidden [&_span]:p-0 mr-0" value='default_4'>
                                        <div className={cn("w-8 h-8 rounded-full overflow-hidden", backgroundConfig.default_4)}></div>
                                    </Radio>
                                    <Radio className="[&_.ant-radio]:hidden [&_span]:p-0 mr-0" value='default_5'>
                                        <div className={cn("w-8 h-8 rounded-full overflow-hidden", backgroundConfig.default_5)}></div>
                                    </Radio>
                                    <Radio className="[&_.ant-radio]:hidden [&_span]:p-0 mr-0" value='default_6'>
                                        <div className={cn("w-8 h-8 rounded-full overflow-hidden", backgroundConfig.default_6)}></div>
                                    </Radio>
                                    <Radio className="[&_.ant-radio]:hidden [&_span]:p-0 mr-0" value='default_7'>
                                        <div className={cn("w-8 h-8 rounded-full overflow-hidden", backgroundConfig.default_7)}></div>
                                    </Radio>
                                </Radio.Group>
                            </div>
                        </div>
                        <div className="[&_label]:font-semibold [&_label]:text-sm flex justify-between items-center gap-2">
                            <label>Canvas</label>
                            {/* <Select
                                defaultValue={ratio}
                                style={{ width: '50%' }}
                        ...
                        {modelFrame.includes(frame) &&
                            <div className="flex items-center justify-between text-xs">
                                <label></label>
                                <div>
                                    <Radio.Group defaultValue={fit} onChange={(e) => setFit(e.target.value as 'cover' | 'contain' | 'fill')} size="small">
