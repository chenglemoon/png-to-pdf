...
import { cn, computedSize, scatterArray } from '@/lib/beautifier-utils';
import { type ImageSize } from '@/lib/beautifier-useImageColor';
...
export const DrawerSide = ({ showMore, onChange, onSelectChange, value, photoUrl, imgColors, imgSize }: DrawerSideProps) => {
    const realWidth = useMemo(() => {
        if (!imgSize?.width || !imgSize?.height) return '96';
        const { width } = computedSize(imgSize.width, imgSize.height, 64, 48);
        return width;
    }, [imgSize]);
    const {options, lists} = useMemo(() => {
        const options: Array<{ key: string; value?: string }> = [];
        const lists: Array<{ key: string; value?: string }> = [];
