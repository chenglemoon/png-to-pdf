...
interface SelectDropdownProps {
    defaultValue: SelectDropdownItem;
    onClick: (item: SelectDropdownItem | undefined) => void;
}

export const SelectDropdown = ({ defaultValue, onClick }: SelectDropdownProps) => {
    ...
    return (
        <Dropdown menu={{ items, style: {width: '300px'}, onClick: handleClick, selectedKeys: [defaultValue.key] }} trigger={['click']} placement="bottomLeft">
            <Button icon={defaultValue.icon} style={{ minWidth: 'auto' }}><div className="inline-block [&_.text-xs]:hidden">{defaultValue.label}</div></Button>
        </Dropdown>
    );
}