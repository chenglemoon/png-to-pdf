...
export const WidthDropdown = ({ defaultValue, onClick }: WidthDropdownProps) => {
    const handleClick = ({ key }: { key: string | number }) => {
        onClick(Number(key));
    }
    return (
        <Dropdown menu={{ items, onClick: handleClick, selectedKeys: [String(defaultValue)] }} trigger={['click']} placement="bottom">
